...
export function ListenerShell({ children }: { children: React.ReactNode }) {
  ...
  </aside><header className="sticky top-0 z-30 border-b border-[var(--line)] bg-[var(--paper)]/90 px-4 py-3 backdrop-blur md:ml-60 md:px-8"><div className="flex items-center gap-3"><button type="button" onClick={() => setMobileOpen(true)} aria-label="মেনু খুলুন" aria-expanded={mobileOpen} className="md:hidden"><Icon name="menu" size={22} /></button><form role="search" onSubmit={submitSearch} className="relative max-w-xl flex-1"><Icon name="search" size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="বই, লেখক বা বিষয় খুঁজুন" className="search-field" placeholder="বই, লেখক বা বিষয় খুঁজুন" /></form><select aria-label="বিষয় অনুযায়ী দেখুন" value={pathname.startsWith("/genre/") ? pathname.split("/").pop() || "" : ""} onChange={(event) => event.target.value && router.push(`/genre/${event.target.value}`)} className="hidden rounded-xl border border-[var(--line)] bg-transparent px-3 py-2 text-xs font-bold text-[var(--muted)] sm:block"><option value="">সব ধরন</option>{genreOptions.map((genre) => <option key={genre.id} value={genre.id}>{genre.name}</option>)}</select><UserMenu /></div></header><main className="app-main min-h-screen">{children}</main></div>;
}
