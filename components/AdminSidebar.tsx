...
export function AdminSidebar(){
 ...
 return <>
  ...
  <aside className={`fixed inset-y-0 left-0 z-40 w-60 border-r border-[#45514a] bg-[#303b36] p-6 text-white transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
   <Link href="/" className="flex items-center gap-3" onClick={()=>setOpen(false)}><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#d5a16a] text-lg font-bold text-[#303b36]">ব</span><span className="serif text-lg font-bold">বই কথা</span></Link>
