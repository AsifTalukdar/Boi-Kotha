...
export function ListenerShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen pb-24"><aside className={`fixed inset-y-0 left-0 z-40 w-60 border-r border-[var(--line)] bg-[#f7efe4] p-6 transition-transform md:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
    <div className="mb-14 flex items-center justify-between"><Link href="/" className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--maroon)] text-lg text-[#f8d5a5]">অ</span><span className="serif text-lg font-bold">[PROJECT NAME]</span></Link><button type="button" onClick={() => setMobileOpen(false)} className="md:hidden"><Icon name="close" size={18} /></button></div>
